...
class ApacheCommonsCliCommandLineParser : CommandLineParser {
    ...
    private val options = Options().apply {
        ...
        addOption(Option("m", "mnemonic", true, "bip39 mnemonic phrase for account generation (hard-coded default used if no account or mnemonic specified)"))
        addOption(Option("a", "account", true, "primary key of accounts to create. This argument can be repeated multiple times"))
        ...
        addOption(Option("k", "hardFork", true, "hard fork to use - one of: byzantium, constantinople, petersburg, istanbul (istanbul)"))
    ...
    override fun parseCommandLine(args: Array<String>): CommandLineParseResult {
        return try { // parse the command line arguments
            ...
            val cmdLine = defaults.copy(
                ...
                mnemonic = nullableStringValue("mnemonic", parsed),
                defaultBalanceEther = bigIntValue("defaultBalanceEther", parsed, defaults.defaultBalanceEther),
                accounts = stringListValue("account", parsed, defaults.accounts),
    ...
    private fun nullableStringValue(argName: String, parsedCommandLine: ApacheCommandLine) =
        parsedCommandLine.getOptionValue(argName)
    ...
    private fun stringListValue(argName: String, parsedCommandLine: ApacheCommandLine, default: List<String>) =
        parsedCommandLine.getOptionValues(argName)?.toList() ?: default
