...
class Executor {
    fun execute(executionCtx: ExecutionContext): ExecutionContext = with(executionCtx) {
        ...
        when {
            ...
            isModifyInStaticCall(opcode, currentCallContext) -> HaltOps.fail(
                executionCtx,
                EvmError(ErrorCode.STATE_CHANGE_STATIC_CALL, "state change not allowed in static call with $opcode")
            )
            else -> processOpcode(executionCtx)
        }
    ...
    private fun isModifyInStaticCall(opcode: Opcode?, currentCallContext: CallContext) =
        currentCallContext.type == CallType.STATICCALL && !Opcode.isAllowedInStatic(opcode)

    private fun isStackUnderflow(opcode: Opcode?, callCtx: CallContext): Boolean {
        val opcodeLength = opcode?.numBytes ?: 0
        return (callCtx.currentLocation + opcodeLength > callCtx.code.size)
    }

    private fun processOpcode(currentContext: ExecutionContext): ExecutionContext = with(currentContext) {
        val byteCode = currentCallContext.code[currentLocation]
        val opcode = Opcode.byCode[byteCode]
