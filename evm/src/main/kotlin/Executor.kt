...
data class Word(val data: List<Byte>) {
    ...
    companion object {
        ...
        fun coerceFrom(byte: Byte) = Word(Byte.Zero.repeat(31) + listOf(byte))
        ...
        fun coerceFrom(str: String): Word = coerceFrom(stripHexPrefix(str).chunked(2).map { Byte(it) })
    }
}
...
class Executor {
    fun execute(currentContext: ExecutionContext, startContext: ExecutionContext): ExecutionContext = with(currentContext) {
